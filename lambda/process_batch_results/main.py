...
def handler(event, context):
    ...
    try:
        ...
        if "Items" not in response or not response["Items"]:
            ...
            return {"statusCode": 200, "body": json.dumps({"message": "No pending batches"})}
        ...
        for item in response["Items"]:
            ...
            if url == "__metadata__":
                # 這是 metadata 記錄
                batches_by_id[batch_id] = [item["url"] for item in sorted(item["metadata"], key=lambda x: x["request_index"])]
            else:
        ...
        return {"statusCode": 200, "body": json.dumps({"message": "Batch processing check completed"})}

    except Exception as e:
        print(f"Error: {str(e)}")
        return {"statusCode": 500, "body": json.dumps({"message": "Error checking batch requests", "error": str(e)})}
