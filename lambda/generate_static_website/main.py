...
def handler(event, context):
    ...
    try:
        ...
        if not request_date:
            return {"statusCode": 400, "body": json.dumps({"message": "date is required"})}

        # Retrieve stats data from DynamoDB
        response = stats_table.get_item(Key={"date": request_date})

        if "Item" not in response:
            return {"statusCode": 404, "body": json.dumps({"message": f"Statistics data not found: {request_date}"})}
        ...
        if not chart_s3_bucket:
            return {"statusCode": 404, "body": json.dumps({"message": f"No charts found: {request_date}"})}
        chart_s3_keys = item.get("chart_s3_keys", [])
        chart_urls = [f"https://{cloudfront_domain}/{key}" for key in chart_s3_keys]
        ...
        try:
            ...
            s3_client.put_object(
                ...
                Metadata={"date": request_date, "generated_at": datetime.now().isoformat()},
            )
            ...
            s3_client.put_object(
                ...
                Metadata={"date": request_date, "generated_at": datetime.now().isoformat(), "is_latest": "true"},
            )
            ...
            traceback.print_exc()
            return {"statusCode": 500, "body": json.dumps({"message": "Failed to upload HTML to S3", "error": str(e)})}

        # Update DynamoDB with HTML URL
        stats_table.update_item(
            Key={"date": request_date},
            UpdateExpression="SET s3_html_url = :html, html_generated_at = :ts",
            ExpressionAttributeValues={":html": html_url, ":ts": int(datetime.now(timezone.utc).timestamp())},
        )
        ...
        traceback.print_exc()
        return {"statusCode": 500, "body": json.dumps({"message": "Error generating static website", "error": str(e)})}
