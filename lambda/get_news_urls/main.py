    ...
    try:
        ...
        if news_articles := google.get_news(category, category_id, location, section_id):
            news_articles = [
                ...
                and (
                    _get_domain(x.url) != "finance.ettoday.net" or _get_first_path(x.url).lower() == "amp"
                )  # https://finance.ettoday.net/news/3089355 這種不符合格式
            ]
            ...
            for article in news_articles:
                ...
                try:
                    sqs_client.send_message(
                        ...
                        MessageAttributes={
                            "url": {"StringValue": article.url, "DataType": "String"},
                            "title": {"StringValue": article.title, "DataType": "String"},
                        },
                    )
            ...
            return {
                "statusCode": 200,
                "body": json.dumps({"message": "News URLs collected successfully", "count": stored_count, "urls": stored_urls}),
            }
        else:
            print("No news articles found")
            return {"statusCode": 404, "body": json.dumps({"message": "No news articles found"})}

    except Exception as e:
        print(f"Error: {str(e)}")
        return {"statusCode": 500, "body": json.dumps({"message": "Error collecting news", "error": str(e)})}
