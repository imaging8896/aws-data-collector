...
def handler(event, context):
    ...
    try:
        ...
        print(f"Error: {str(e)}")
        return {"statusCode": 500, "body": json.dumps({"message": "Error collecting news content", "error": str(e)})}
...
def process_news_url(url):
    ...
    try:
        ...
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "News content collected and batch job submitted", "url": actual_news_url}),
        }
    except Exception as e:
        print(f"Error: {str(e)}")
        return {
            "statusCode": 500,
            "body": json.dumps({"message": f"Failed to get news content from {url}", "error": str(e)}),
        }
